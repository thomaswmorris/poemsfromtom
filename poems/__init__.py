...
class Curator():
    ...
    def make_stats(self, order_by=None, ascending=True, force_rows=True, force_cols=True):
        ...
        for uauthor in self.unique_authors:
            name, birth, death, nationality, link = POEMS[uauthor]['metadata'].values()

            elapsed = (ttime.time() - self.history['timestamp'][self.history.author==uauthor].max()) / 86400 
            n_poems = len(POEMS[uauthor]['poems'])
    ...
    def get_poem(
        ...
        if 'history' in weight_schemes:
            ...
            for uauthor in self.unique_authors:
                ...
                days_since_last_sent_weight = 1 / (1 + np.exp(-.25 * (self.stats.days_since_last_sent.fillna(365).loc[uauthor] - 28))) # after four weeks, the weight is 0.5
